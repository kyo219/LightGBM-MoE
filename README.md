...
**Setup**: 100 Optuna trials, 5-fold time-series CV, early stopping (50 rounds).

### RMSE Comparison

| Dataset | Standard | MoE | MoE-PE | Best Improvement |
|---------|----------|-----|--------|------------------|
| Synthetic | 5.2559 | **4.8390** | 5.2509 | +7.9% |
| Hamilton | 0.7230 | **0.7154** | 0.7191 | +1.0% |
...
|---------|-------|----------|--------------------|-----------------------|----------------|-------------------|
| Synthetic | 2 | 3 | 1.00/1.00 | 1.00/1.00 | 57.8% | 50.2% |
| Hamilton | 3 | 4 | 0.51/0.59 | 0.46/0.72 | 51.6% | 51.4% |
...
**Key Findings**:
- MoE achieves **+7.9% improvement** on Synthetic data (feature-driven regimes)
- Hamilton (latent regimes) shows Expert correlation 0.51-0.59 with moderate differentiation
- Latent regime data shows limited improvement as expected (regime not fully determinable from X)

### Selected Hyperparameters (Synthetic Dataset)

**MoE (Shared Tree Structure):**
- num_experts: 2, max_depth: 12, num_leaves: 127, learning_rate: 0.087, alpha: 0.90

**MoE-PerExpert (Per-Expert Tree Structure):**
- num_experts: 3, learning_rate: 0.131, alpha: 1.02
...
**設定**: 100 Optunaトライアル、5分割時系列CV、early stopping (50 rounds)。
...
**重要な発見**:
- MoEがSyntheticデータで **+7.9%の改善** （特徴量ベースのレジーム）
- Hamilton（潜在レジーム）ではExpert相関0.51-0.59で中程度の分化を達成
- 潜在レジームデータでは改善が限定的（レジームがXから完全には決定できないため、想定通り）
