...
|---------|----------|-----|--------|------------------|
| Synthetic | 5.0823 | **4.5610** | 5.1176 | +10.3% |
| Hamilton | 0.7172 | **0.7172** | 0.7175 | +0.0% |
...
|---------|-------|----------|--------------------|-----------------------|----------------|-------------------|
| Synthetic | 3 | 2 | -0.28/-0.02 | 0.90/0.90 | 68.3% | 53.8% |
| Hamilton | 2 | 3 | 0.49/0.49 | 0.14/0.74 | 63.0% | 57.4% |
...
**Key Findings**:
- MoE achieves **+10.3% improvement** on Synthetic data with expert correlation of **-0.28** (experts specialize on different regimes)
- MoE achieves **68.3% regime accuracy** on Synthetic data (2 true regimes correctly identified)
- Hamilton (latent regimes): MoE matches Standard GBDT with **63.0% regime accuracy** and expert correlation **0.49** (good differentiation despite latent regime structure)
- Latent regime data shows limited RMSE improvement as expected (regime not fully determinable from X), but MoE successfully differentiates experts

### Selected Hyperparameters (Synthetic Dataset)

**MoE (Shared Tree Structure):**
- num_experts: 3, max_depth: 3, num_leaves: 37, learning_rate: 0.169, alpha: 0.61, gate_depth: 5, gate_leaves: 31

**MoE-PerExpert (Per-Expert Tree Structure):**
- num_experts: 2, learning_rate: 0.105, alpha: 1.78, gate_depth: 3, gate_leaves: 48
...
| Hamilton | 500 | 4 | Latent time-varying regime probabilities |
...
**重要な発見**:
- MoEがSyntheticデータで **+10.3%の改善**、Expert相関 **-0.28**（Expertが異なるレジームに特化）
- MoEが **68.3%のレジーム精度** を達成（2つの真のレジームを正しく識別）
- Hamilton（潜在レジーム）: MoEがStandard GBDTと同等のRMSEを達成、**レジーム精度63.0%**、Expert相関**0.49**（潜在レジームにもかかわらず良好な分化）
- 潜在レジームデータではRMSE改善は限定的（レジームがXから完全には決定できないため、想定通り）だが、Expertの分化には成功
