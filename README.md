...
|---------|----------|-----|--------|------------------|
| Synthetic | 5.0438 | **4.5858** | 4.7873 | +9.1% |
| Hamilton | 0.7180 | 0.7203 | **0.7160** | +0.3% |
...
|---------|-------|----------|--------------------|-----------------------|----------------|-------------------|
| Synthetic | 3 | 3 | -0.70/0.02 | -0.15/0.15 | 84.0% | 67.6% |
| Hamilton | 3 | 2 | 0.58/0.79 | 0.15/0.15 | 52.2% | 53.4% |
...
**Key Findings**:
- MoE achieves **+9.1% improvement** on Synthetic data with expert correlation of **-0.70** (experts predict opposite regimes!)
- MoE achieves **84.0% regime accuracy** on Synthetic data (2 true regimes correctly identified)
- Hamilton (latent regimes) shows MoE-PE expert correlation 0.15 with good differentiation
- Latent regime data shows limited RMSE improvement as expected (regime not fully determinable from X)

### Selected Hyperparameters (Synthetic Dataset)

**MoE (Shared Tree Structure):**
- num_experts: 3, max_depth: 4, num_leaves: 104, learning_rate: 0.110, alpha: 0.57

**MoE-PerExpert (Per-Expert Tree Structure):**
- num_experts: 3, learning_rate: 0.099, alpha: 1.74
...
**重要な発見**:
- MoEがSyntheticデータで **+9.1%の改善**、Expert相関 **-0.70**（Expertが逆のレジームを予測！）
- MoEが **84.0%のレジーム精度** を達成（2つの真のレジームを正しく識別）
- Hamilton（潜在レジーム）ではMoE-PEのExpert相関0.15で良好な分化を達成
- 潜在レジームデータではRMSE改善が限定的（レジームがXから完全には決定できないため、想定通り）
